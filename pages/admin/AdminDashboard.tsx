...
import { MockSubmission, UploadedContent, GoverningBodyMember, OfficialSignatures, DataEntryRecord, GalleryImage, PersonDomain, HeaderAlert } from '../../types';
...
export const AdminDashboard: React.FC = () => {
  ...
  const handleAddAlert = () => {
    ...
    updateHeaderAlerts([...headerAlerts, { id: Date.now().toString(), text: newAlertText, isActive: true }]);
    setNewAlertText('');
  };

  const toggleAlert = (id: string) => {
    updateHeaderAlerts(headerAlerts.map(a => a.id === id ? { ...a, isActive: !a.isActive } : a));
  };

  const deleteAlert = (id: string) => {
    updateHeaderAlerts(headerAlerts.filter(a => a.id !== id));
  };
        ...
        <div className="p-12 space-y-12 pb-32">
          ...
          {activeTab === 'timeline' && (
            <div className="animate-fadeIn max-w-4xl mx-auto space-y-12">
               <div className="bg-white border-4 border-black rounded-[4rem] p-16 shadow-2xl">
                  <h3 className="text-4xl font-serif font-black uppercase text-kku-blue mb-10 border-b-4 border-kku-gold pb-4 inline-block">Header Alert Master</h3>
                  <p className="text-gray-400 font-bold uppercase text-[10px] tracking-widest mb-10">Manage vibrant blinking notices on top right of the portal.</p>
                  ...
                  <div className="flex gap-4 mb-12">
                    <input 
                      ...
                      placeholder="Enter Vibrant Blink Message (e.g. Upcoming Exam)"
                      className={inputCls}
                    />
                    <button onClick={handleAddAlert} className="bg-kku-blue text-white px-10 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest whitespace-nowrap border-2 border-white shadow-xl hover:bg-black transition-all">+ Add Alert</button>
                  </div>

                  <div className="space-y-6">
                    {headerAlerts.map(alert => (
                      <div key={alert.id} className="p-6 bg-gray-50 border-2 border-black/5 rounded-3xl flex items-center justify-between shadow-md">
                        <div className="flex items-center gap-6">
                          <button onClick={() => toggleAlert(alert.id)} className={`w-6 h-6 rounded-full border-2 border-white shadow-lg ${alert.isActive ? 'bg-green-500' : 'bg-gray-300'}`}></button>
                          <span className={`font-black uppercase text-xs ${alert.isActive ? 'text-kku-blue' : 'text-gray-400 line-through'}`}>{alert.text}</span>
                        </div>
                        <button onClick={() => deleteAlert(alert.id)} className="text-red-700 font-black uppercase text-[9px] hover:underline">Erase</button>
                      </div>
                    ))}
                  </div>
               </div>
